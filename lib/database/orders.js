...
/* Find all menu_items associated to a order */
const findMenuItemsForOrder = function(order_id) {
  const query = `SELECT menu_items.* FROM order_item JOIN menu_items ON menu_items.id = order_item.menu_item_id WHERE order_item.order_id = $1;`;
...
const createOrderItem = function(order_id, menu_item_id) {
  const query = `INSERT INTO orderitem (order_id, menu_item_id) values ($1, $2);`;
  const queryParams = [order_id, menu_item_id];
  return pool.query(query,queryParams);
};

const addOrderItem = function(order_id, menu_item_id) {
  const query = `UPDATE orderitem SET quantity = quantity + 1 WHERE order_id = $1 AND menu_item_id = $2;`;
  const queryParams = [order_id, menu_item_id];
  return pool.query(query,queryParams);
};
