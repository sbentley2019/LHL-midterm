...
const findOrderId = function(user_id, restaurant_id) {
  const query = `SELECT * FROM orders WHERE user_id = $1 AND restaurant_id = $2`;
...
/**
 * Returns Order details given order_id
 * @param {*} order_id
 */
const findOrderDetails = function(order_id) {
  const query = `SELECT orders.id, users.first_name, users.last_name, orders.start_time, orders.estimated_end_time FROM orders
...
const processOrder = function(order_id, total_cost) {
  ...
  return pool.query(query, queryParams).then(res => res.rows[0]).catch(err => console.log('ERROR: ', err));
