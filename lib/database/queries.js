module.exports = pool => {
  ...
  const getRestaurantWithId = function(id) {
    return pool.query(`SELECT * FROM restaurants WHERE id = $1`, [id]).then(
      res => {
        return res.rows[0];
      },
      rej => null
  ...
  const getMenuItemsWithId = function(id) {
    return pool.query(`SELECT * FROM menu_items WHERE id = $1`, [id]).then(
      res => {
        return res.rows[0];
      },
      rej => {
  ...
  const getAllMenuItemsWithRestaurantId = function(id) {
    return pool
      .query(`SELECT * FROM menu_items WHERE restaurant_id = $1`, [id])
      .then(
        res => {
          return res.rows[0];
        },
