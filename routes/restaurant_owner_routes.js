...
module.exports = function(db) {
  ...
  router.get("/getOrders", (req, res) => {
    restaurants.findRestaurantIdByOwnerId(req.session.user_id).then(resId => {
      orders.findByRestaurant(resId.id).then(rows => {
  ...
  router.post("/confirm_order", (req, res) => {
    ...
    orders.findById(order_id).then(order => {
      orders.updateOrder("Accepted", order.id, "current_status").then(rows => {
        tclient.messages
          .create({
  ...
  router.post("/cancel_order", (req, res) => {
    ...
    tclient.messages
      ...
      .then(message => {
        orders
          .updateOrder("Rejected", order_id, "current_status")
          .then(rows => {
            res.end();
          });
      });
    ...
    restaurants.findRestaurantIdByOrderId(order_id).then(restaurant => {
      ...
      tclient.messages
        ...
        .then(message => {
          ...
          orders
            .updateOrder("Ready", order_id, "current_status")
            .then(row => {
              orders.updateOrder(
                req.body.order_time,
                order_id,
  ...
  router.get("/:id/userName", (req, res) => {
    menu_items
      .findNameByOrderId(req.params.id)
      .then(name => {
        console.log(name);
        res.status(200);
        res.json(name);
