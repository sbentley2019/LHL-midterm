...
module.exports = db => {
  ...
  router.get("/:id/loadMenuItems", (req, res) => {
    menu_items
      .findByOrderId(req.session.order_id)
      .then(menu_item => {
        res.status(200);
        res.json(menu_item);
      })
  ...
  router.get("/:id/orderTotal", (req, res) => {
    menu_items
      .totalOrder(req.session.order_id)
      .then(total => {
        res.status(200);
        res.json(total);
      })
  ...
  router.post("/checkout", (req, res) => {
    menu_items.totalOrder(req.session.order_id).then(total_cost => {
      console.log(
        `***Order #${req.session.order_id} has been placed, you will be charged $ ${total_cost} when the order is accepted.***`
      );
      orders.processOrder(req.session.order_id, total_cost);

      req.session.msg =
        "Order has been placed! Please check your phone for updates";
  ...
  router.post("/:id", (req, res) => {
    res.status(200);
    menu_items
      .findById(req.session.order_id, req.body.menu_item)
      .then(order => {
        // Check if menu_item with order_id exist
        if (order) {
          console.log("Adding to existing order");
          orders
            .addOrderItem(req.session.order_id, req.body.menu_item)
            .then(data => res.json(data));
        } else {
          orders
            .createOrderItem(req.session.order_id, req.body.menu_item)
            .then(data => res.json(data));
        }
      });
  });
